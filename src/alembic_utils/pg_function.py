...
class PGFunction(ReplaceableEntity):
    ...
    * **definition** - *str*:  The remainig function body and identifiers
    ...
    def __init__(self, signature: str, definition: str, schema: str = "public", is_proc=False):
    ...
    def to_sql_statement_create(self):
        ...
        return sql_text(
            f"CREATE {entity_type} {self.literal_schema_prefix}{self.literal_signature} {self.definition}"
        )
    ...
    def to_sql_statement_drop(self, cascade=False):
        ...
        full_sql = f"CREATE {entity_kind} {self.literal_schema_prefix}{self.literal_signature} {self.definition}"
    ...
    def to_sql_statement_create_or_replace(self):
        ...
        entity_type = "PROCEDURE" if self.is_proc else "FUNCTION"
        yield sql_text(
            f"CREATE OR REPLACE {entity_type} {self.literal_schema_prefix}{self.literal_signature} {self.definition}"
        )
