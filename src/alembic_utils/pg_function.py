...
class PGFunction(ReplaceableEntity):
    ...
    * **definition** - *str*:  The remainig function body and identifiers
    ...
    def get_database_definition(
        self: "PGFunction", sess: Session, dependencies: Optional[List["ReplaceableEntity"]] = None
    ) -> "PGFunction":  
        ...
        with simulate_entity(sess, self, dependencies) as sess:
            ...
            sql = sql_text("""
                    ...
                    where n.nspname = :schema
                        and concat(p.proname, '(', pg_get_function_arguments(p.oid), ')') ilike :signature
                    """)
            ...
            return PGFunction.from_sql(info_row[3])
